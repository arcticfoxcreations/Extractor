...
The API address is no longer a runtime setting in the UI — there is
deliberately no "Backend" button for a visitor to misconfigure. It is
hardcoded in `frontend/js/config.js`:

```js
var PRODUCTION_API = 'https://your-backend.onrender.com';
var LOCAL_API = 'http://localhost:8080'; // used automatically on localhost
```

Edit `PRODUCTION_API` to your backend's URL and commit. Anyone who opens
the deployed page gets that address automatically; running the frontend
from `localhost` uses `LOCAL_API` instead, so local development against a
local backend needs no edits.
...
Note that the frontend now pings `/api/health` on page load specifically to
wake a sleeping Render dyno early, and shows "Waking up the server…" instead
of a bare timeout while a cold start is in progress — this is expected
behaviour on the free tier, not a bug.

**A file attach fails with "That file type is not supported."** Only `.txt`,
`.md`, `.pdf`, `.docx`, `.xlsx`, `.xls` and `.pptx` are accepted — legacy
binary `.doc`/`.ppt` and image formats are rejected on purpose. Re-save the
