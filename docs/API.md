...
Extracts plain text from an uploaded document, for pasting into the reader
without retyping it. Send as `multipart/form-data` with the file under the
field name `file`.

Accepted types: `.txt`, `.md`, `.pdf`, `.docx`, `.xlsx`, `.xls`, `.pptx`
(legacy binary `.doc`/`.ppt` and any image format are rejected). Files are
capped at 15 MB (`MAX_FILE_SIZE_BYTES`).
...
The upload is parsed entirely in memory (`multer` memory storage) and is
never written to disk — the buffer is discarded as soon as the response is
sent, so nothing about the file persists on the server.

**Errors:** `missing_file` (400), `unsupported_file_type` (415),
`file_too_large` (413), `extraction_failed` / `no_text_found` (422),
`rate_limited_file` (429).
...
| `/api/extract-file` | 15 requests per minute per IP |
